...
// Unlike fetchHomeAssistantStates (capped, used for the actual AI prompt), the picker needs
// every entity to choose from — otherwise domains like camera can silently go missing.
async function listAllHomeAssistantEntities(configNode, signal) {
  const { baseUrl } = prepareConfig(configNode);
  const cacheKey = `${configNode.id || "default"}:${baseUrl}`;
  const cached = listCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) return cached.entities;

  const response = await requestHomeAssistant(configNode, "/api/states", {}, signal);
  const states = await response.json();
  if (!Array.isArray(states)) throw new Error("Home Assistant returned an invalid states response");
  ...
  listCache.set(cacheKey, { expiresAt: Date.now() + CACHE_TTL_MS, entities });
  return entities;
}
...
module.exports = { fetchHomeAssistantStates, listAllHomeAssistantEntities, fetchHomeAssistantServices, callHomeAssistantService, prepareConfig, compactStates, MAX_ENTITIES, MAX_CONTEXT_CHARS, CACHE_TTL_MS };