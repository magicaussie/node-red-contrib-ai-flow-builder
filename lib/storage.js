...
class Storage {
  constructor(userDir) {
    ...
    this.auditFile = path.join(this.rootDir, "audit.log");
  ...
  _backupPath(id) {
    if (!/^[a-f0-9-]+$/i.test(id)) throw new Error("invalid backup id");
  ...
  async saveBackup({ conversationId, reason, flowJson }) {
    const id = crypto.randomUUID();
    const backup = {
      id,
      conversationId: conversationId || null,
      reason: reason || "AI apply",
      createdAt: Date.now(),
      flowJson: Array.isArray(flowJson) ? flowJson : []
    };
    await this._writeAtomic(this._backupPath(id), JSON.stringify(backup, null, 2));
    await this.appendAudit({ type: "backup", conversationId: backup.conversationId, backupId: id, reason: backup.reason, nodeCount: backup.flowJson.length });
    return { id, conversationId: backup.conversationId, reason: backup.reason, createdAt: backup.createdAt, nodeCount: backup.flowJson.length };
  }

  async listBackups(limit = 20) {
    const entries = await fs.readdir(this.backupDir).catch(() => []);
    const out = [];
    for (const f of entries) {
      if (!f.endsWith(".json")) continue;
      try {
        const backup = JSON.parse(await fs.readFile(path.join(this.backupDir, f), "utf8"));
        out.push({ id: backup.id, conversationId: backup.conversationId, reason: backup.reason, createdAt: backup.createdAt, nodeCount: (backup.flowJson || []).length });
      } catch (_) {}
    }
  ...
  async getBackup(id) {
    return JSON.parse(await fs.readFile(this._backupPath(id), "utf8"));
  }

  async appendAudit(entry) {
    const record = { ts: Date.now(), ...entry };
    await fs.appendFile(this.auditFile, `${JSON.stringify(record)}\n`);
    return record;
  }

  async listAudit(limit = 100) {
    const raw = await fs.readFile(this.auditFile, "utf8").catch(() => "");
